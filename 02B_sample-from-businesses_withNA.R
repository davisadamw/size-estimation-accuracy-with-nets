...
sample_summary <- function(run, size, source_data = nets_size_2013, summary_col = Emp13, weight = NULL) {
  ...
  # sample businesses, apply random NA, and collapse to uniques
  s <- source_data %>% 
    sample_n(size, replace = TRUE, weight = {{ weight }}) %>% 
    mutate(summary_rept = if_else(runif(n()) < 0.25, NA_real_, {{ summary_col }})) %>% 
    group_by_at(vars(-summary_rept)) %>% 
    summarize(n = n(),
              emp_rept = mean(summary_rept, na.rm = TRUE)) %>% 
    ungroup()
  ...
  s %>% 
    summarize(tot_ests_sample = n(),
              ...
              tot_emps_rept   = sum(emp_rept, na.rm = TRUE),
              na_emps_in_sample = sum(is.nan(emp_rept)),
              mean_emps_sample = mean({{ summary_col }}),
              trmn_emps_sample = mean({{ summary_col }}, trim = 0.05),
              med_emps_sample = median({{ summary_col }}),
              mean_emps_rept = mean(emp_rept, na.rm = TRUE),
              trmn_emps_rept = mean(emp_rept, trim = 0.05, na.rm = TRUE),
              med_emps_rept  = median(emp_rept, na.rm = TRUE)) %>% 
    mutate(run_no = run,
           sample_size = size,
...
sample_results %>% 
  write_rds('Data/CA_business_subsamples_withNAs.rds', compress = 'gz')
...
sample_results %>% 
  ...
  geom_point() +
  geom_point(aes(y = tot_emps_rept), color = 'red') +
  #geom_smooth() +
...
sample_results %>% 
  ...
  theme_bw()
ggsave('mean_size_methods.png')

sample_results %>% 
  mutate(sample_frac = sample_size / sum(nets_size_2013$Emp13)) %>% 
  select(sample_frac, mean_emps_sample, mean_emps_rept) %>% 
  pivot_longer(-sample_frac,
               names_to = c('.value', 'source'),
               names_pattern = '([a-z]+)_emps_([a-z]+)') %>% 
  mutate(source = if_else(source == 'rept', 'reported', 'full sample')) %>% 
  ggplot(aes(sample_frac, mean, group = source, color = source)) +
  geom_point() +
  #geom_smooth() +
  geom_hline(yintercept = mean(nets_size_2013$Emp13)) +
  scale_x_continuous(labels = scales::percent_format(1)) +
